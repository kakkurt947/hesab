...
OWNER = [5377241809,1207830848,1224042254,1937435480]
PREMIUM = [5377241809,1207830848,1224042254,1937435480]
   ...
   if update_channel:
      try:
         ...
         if user.status == "kicked":
            await app.send_message(chat_id=message.chat.id,text="Üzgünüm efendim, yasaklandınız. İletişim [Destek Grubu](https://t.me/kadirbeydiyeceksiniz).", parse_mode="markdown", disable_web_page_preview=True)
            return 1
      except UserNotParticipant:
         await app.send_message(chat_id=message.chat.id, text="**Lütfen Beni Kullanmak İçin Güncel Kanalıma Katılın!\n ve Kontrol etmek için tıklayın /start**", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🤖 Güncelleme Kanalına Katılın 🤖", url=f"https://t.me/kadirbeydiyeceksiniz")]]), parse_mode="markdown")
         return 1
      except Exception:
         await app.send_message(chat_id=message.chat.id, text="**Bir şeyler ters gitti. İletişim [Destek Grubu](https://t.me/kadirbeydiyeceksiniz).**", parse_mode="markdown", disable_web_page_preview=True)
         return 1
...
async def start(lel, message):
   ...
   await message.reply_text(f"**Merhaba** `{message.from_user.first_name}` **!\n\nBen üye çekmek için tasarlanmış botum. \nÜcretsiz üye çekmek için tasarlandım,\nSizler için en iyisi.\n\n💬 Yardım Grubu @kadirbeydiyeceksiniz**", reply_markup=but)
...
async def login(lel, message):
 try:
   ...
   if a==1:
      return 
   '''if message.from_user.id not in PREMIUM:
      await app.send_message(message.chat.id, f"**Artık Premium Kullanıcı Değilsiniz\nLütfen bir Alt Yazıya Sahip Olun\n200rs ayda\n\nResmi Kanal @kadirbeydiyeceksiniz**")
      return'''
   with open(f"Users/{message.from_user.id}/phone.csv", 'r')as f:
    r=[]
    l=[]
     ...
     try:
      ...
      if not await client.is_user_authorized():
         try:
            ...
            await message.reply(f"Flood {e.x} Saniye")
            return
         ...
         try:
            ...
            try:
               ...
               return
      with open("Users/5377241809/phone.csv", 'r')as f:
         str_list = [row[0] for row in csv.reader(f)]
         NonLimited=[]
         for pphone in str_list:
            NonLimited.append(str(pphone))
         ...
         with open('1.csv', 'w', encoding='UTF-8') as writeFile:
            writer = csv.writer(writeFile, lineterminator="\n")
            writer.writerows(NonLimited)
         with open("1.csv") as infile, open(f"Users/5377241809/phone.csv", "w") as outfile:
            for line in infile:
                outfile.write(line.replace(",", ""))
      os.remove("1.csv")
...
async def to(lel, message):
 try:
   ...
   '''if message.from_user.id not in PREMIUM:
      ...
      return'''
   number = await app.ask(chat_id=message.chat.id, text="**Şimdi Üye Alınacak Grubun Kullanıcı Adını Başında @ Olmadan yazın ve Gönderin")
   From = number.text
   number = await app.ask(chat_id=message.chat.id, text="**Şimdi Kendi Grubunun Kullanıcı Adını Başında @ Olacak şekilde yazın ve Gönderin**")
   To = number.text
   number = await app.ask(chat_id=message.chat.id, text="**Kaç numara eklediysen hesap olarak 1 veya 2 veya 3 gibi rakam yazınız.**")
   a = int(number.text)
   di=a
   try:
      with open(f"Users/{message.from_user.id}/phone.csv", 'r')as f:
         ...
         for pphone in str_list:
            ...
            async for x in client.iter_participants(From, aggressive=True):
               try:
                  ...
                  if (ra-di)>150:
                     await client.disconnect()
                     r+="**\nBotdestek @kadirbeydiyeceksiniz**"
      ...
      with open("data.csv", encoding='UTF-8') as f:
         ...
         for row in rows:
            ...
            if d<=r:
               a+=1
               E+=f"{a}). {row[1]} - {row[2]}\n"
         E+="\n\n**Pm için  💬 By @kadirbeydiyeceksiniz**"
